...
class App(ctk.CTk):
    ...
    def _pivot_data(self, df):
        """Transforma el DataFrame de formato largo a formato ancho (crosstab), incluyendo UUID y usando el importe seleccionado."""
        if df.empty:
            return pd.DataFrame()
        ...
        try:
            ...
            # Índices: Dependencia, EmisorRFC, ReceptorRFC, UUID
            pivot_df = pd.pivot_table(
                df, 
                values=importe_columna, 
                index=['Dependencia', 'EmisorRFC', 'ReceptorRFC', 'UUID'], 
            ...
            # Sumamos horizontalmente todas las columnas de percepciones (a partir de la quinta columna, índice 4)
    ...
    def show_data_in_treeview(self, df):
        ...
        for col in columns:
            ...
            # Damos más ancho a las columnas clave (Dependencia, EmisorRFC, ReceptorRFC, UUID, Total General)
            self.tree.column(col, width=150 if col in ['Dependencia', 'EmisorRFC', 'ReceptorRFC', 'UUID', 'Total General'] else 100, anchor="w")
